...
begin
  try
        ...
        case Relacionamento.TipoRelacionamento of
          ...
          UmPraMuitos:
            begin
              ...
              Value                  := TabelaFilha.BuscaListaFilhos(ValorBusca.ToInteger, 3);
            end;
        end;
        Propriedade.SetValue(Self, Value);
      end;
    ...
    on E: Exception do
    begin
      if ((Pos('FIELD', UpperCase(E.Message)) > 0) and (Pos('NOT FOUND', UpperCase(E.Message)) > 0) or (Pos('TABLE UNKNOWN', UpperCase(E.Message)) > 0)) and
        (Tentativa < 5) then
      begin
...
function TTabela.BuscaListaFilhos(ValorBusca: integer; Tentativa: smallint): TList<TTabela>;
var
  Propriedade: TRttiProperty;
  Banco: TBanco;
  Value: TValue;
begin
  try
    IBQR.Close;
    IBQR.SQL.Clear;
    IBQR.SQL.Add('SELECT * FROM ' + Self.Nome + ' WHERE ' + Self.CampoBusca + ' = ' + ValorBusca.ToString);
    IBQR.Open;
    //
    IBQR.First;
    ...
    while not IBQR.Eof do
    begin
      Result.Add(TTabela.Create(TFDConnection(Self.IBQR.Connection)));
      for Propriedade in Propriedades.Keys do
      begin
        Value := BuscaValorDataSet(Propriedade);
        Propriedade.SetValue(Pointer(Result[Pred(Result.Count)]), Value);
      end;
      IBQR.Next;
    end;
  except
...
begin
  try
    ...
    begin
      ...
      case Propriedade.PropertyType.TypeKind of
        tkInteger: InsereTabela.AddCamposEValores(Campo.Nome, Propriedade.GetValue(Self).AsInteger);
        tkFloat:
        begin
          if (Propriedade.GetValue(Self).TypeInfo = TypeInfo(TDate))
            or (Propriedade.GetValue(Self).TypeInfo = TypeInfo(TTime))
            or (Propriedade.GetValue(Self).TypeInfo = TypeInfo(TDateTime)) then
          begin
        ...
        tkInt64: InsereTabela.AddCamposEValores(Campo.Nome, Propriedade.GetValue(Self).AsInt64);
        else
          InsereTabela.AddCamposEValores(Campo.Nome, Propriedade.GetValue(Self).AsVariant)
      end;
    end;
    ...
    on E: Exception do
    begin
      if ((Pos('COLUMN UNKNOWN', UpperCase(E.Message)) > 0) or (Pos('TABLE UNKNOWN', UpperCase(E.Message)) > 0)) and
        (Tentativa < 5) then
      begin
        // Caso alguma coluna não exista na tabela, ela será criada
        Banco := TBanco.Create(TFDConnection(IBQR.Connection));
