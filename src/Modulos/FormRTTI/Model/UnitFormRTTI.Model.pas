...
begin
	...
	try
		...
		begin
			...
			begin
				ValorParaComponent(FForm.FindComponent(prpRtti.Name), ObtemRTTIPropertyValue(FTabela, prpRtti.GetAttribute<TLigarCampos>.Campo));
			end;
		end;
	...
	try
		...
		begin
			...
			if not(Response.StatusCode in [200, 201]) then
				raise Exception.Create('Erro ao enviar dados para o servidor!' + sLineBreak + sLineBreak + 'Response: ' + Response.Content + sLineBreak + sLineBreak + 'Error:' + Response.Error + sLineBreak + sLineBreak + 'StatusCode:' + Response.StatusCode.ToString);
		end
		else
...
begin
	...
	try
		...
		begin
			...
			begin
				ValorParaProperty(FTabela, ObtemRTTIProperty(FTabela, prpRtti.GetAttribute<TLigarCampos>.Campo), ObtemValorComponent(FForm.FindComponent(prpRtti.Name)));
			end;
		end;
	finally
		ctxRtti.Free;
		...
		begin
			case prpRttiProp.PropertyType.TypeKind of
				tkUnknown:
					;
				tkInteger:
					prpRttiProp.SetValue(Pointer(Tabela), 0);
				tkChar:
					;
				tkEnumeration:
					;
...
begin
	case Propriedade.PropertyType.TypeKind of
		tkUnknown:
			;
		tkInteger:
			Propriedade.SetValue(Pointer(Tabela), StrToInt(Valor.ToString));
		tkChar:
			;
		tkEnumeration:
			;
		tkFloat:
			begin
				if (Valor.TypeInfo = TypeInfo(TDate)) or (Valor.TypeInfo = TypeInfo(TTime)) or (Valor.TypeInfo = TypeInfo(TDateTime)) then
		...
		tkInterface:
			;
		tkInt64:
			Propriedade.SetValue(Pointer(Tabela), Valor.Cast<Int64>);
		tkDynArray:
			;
		tkClassRef:
			;
